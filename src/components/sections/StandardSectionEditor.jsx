        ...
        <div key={sIdx} className="p-3 rounded-xl bg-slate-900/80 border border-slate-800 space-y-2 text-xs">
          <div className="grid grid-cols-12 gap-2 items-center">
            ...
            <div className="col-span-6 sm:col-span-4">
              ...
              <input
                ...
                className="w-full px-2 py-1.5 bg-slate-950 border border-slate-700 rounded-lg text-xs font-mono font-bold text-indigo-300 focus:outline-none focus:border-indigo-500"
              />
            </div>
            ...
            {/* Botón Eliminar en Móvil / Desktop */}
            ...
            <div className="col-span-12 sm:col-span-4">
              ...
              <input
                ...
                className="w-full px-2 py-1.5 bg-slate-950 border border-slate-700 rounded-lg text-xs text-slate-300 focus:outline-none truncate focus:border-indigo-500"
              />
            </div>
          ...
          <div className="space-y-1.5 pt-1 border-t border-slate-800/60">
            ...
            {sec.schedule.map((slot, bIdx) => (
              <div key={bIdx} className="bg-slate-950 p-2 rounded-lg border border-slate-800 flex flex-wrap items-center gap-1.5 text-xs">
                <select
                  value={slot.day}
                  onChange={(e) => onUpdateSlot(courseId, sIdx, bIdx, 'day', e.target.value)}
                  className="bg-slate-900 border border-slate-700 rounded-lg px-2 py-1.5 text-slate-200 text-[11px] cursor-pointer flex-1 sm:flex-none"
                >
                  {weekDays.map(d => (
                ...
                <div className="flex items-center gap-1 flex-1 sm:flex-none justify-between sm:justify-start">
                  <select
                    ...
                    className="bg-slate-900 border border-slate-700 rounded-lg px-2 py-1.5 text-slate-200 text-[11px] font-mono cursor-pointer"
                  >
